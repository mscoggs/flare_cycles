...
def plot_tvf(KIC, files, num_files, tvf_data, fixed_energy, target_index, **kwargs):
    ...
    for x in range(num_files):
        ...
        if(np.amax(evf_x_energy) >= fixed_energy >= np.amin(evf_x_energy)):
            ...
            err_up, err_dn = calc_error_xi(ffdYAtEnergy, total_duration)
...
def fractional_lum(KIC, files, num_files, **kwargs):
    ...
    for x in range(num_files):
        ...
        log_fractional_lum = np.append(log_fractional_lum, np.log10(np.sum(energy_p)/total_duration))
    ...
    try:
        ...
        plt.scatter(time, log_fractional_lum, c=time, cmap=CMAP, lw=6)

        if(PLOT):
            if(kwargs['save']==True): plt.savefig('log_space_analysis/fractional_lum/'+ str(KIC) + '_frac_lum.png')
...
def plot_evf(KIC, files, num_files, **kwargs):
    ...
    for x in range(num_files):
        ...
        sort = np.argsort(energy_p)
        evf_x_energy = np.log10((energy_p + EPOINT)[sort][::-1])
        log_evf_y_frequency = np.log10((np.arange(1, len(evf_x_energy)+1, 1))/total_duration)
        ...
        err_up, err_dn = calc_error_xi(log_evf_y_frequency[ok68], total_duration)
        #err_up > err_dn, which is why we're using that for the error bars
        err_array = np.append(err_array, err_up)

        if(PLOT):
            plt.plot(evf_x_energy[ok68], log_evf_y_frequency[ok68], lw = 1, c = CMAP(x/float(len(files))))
    ...
    try:
        ...
        if(PLOT):
            ...
            if(kwargs['save']==True): plt.savefig('log_space_analysis/evf/'+ str(KIC) + '_evf.png')
            if(kwargs['showe']==True): plt.show()
            plt.close()
    ...
    return quarterly_evf_x_energy, quarterly_log_evf_y_frequency, popt, perr, total_duration, time, success
...
def mcmc(x,y,yerr, m_guess, b_guess):
    ...
    m = m_mcmc[0]
    b =  b_mcmc[0]
    m_str = str('{:.2e}'.format(m))
    b_str = str('{:.2e}'.format(b))
    label = "m: " + m_str + ", b: " + b_str
    plt.plot(x, m*x+b, color="r", label = label, lw=2, alpha=0.8)
    plt.legend(loc='upper right')
    popt = [ m_mcmc[0], b_mcmc[0]]
    perr = [ b_mcmc[1],  b_mcmc[1]]
    return popt, perr



def plot_evf_sub(KIC, quarterly_evf_energy, quarterly_evf_log_frequency, popt,perr, total_duration, **kwargs):
    '''
    ...
    for q in range(len(quarterly_evf_energy)):
        ...
        err_up, err_dn = calc_error_xi(quarterly_evf_log_frequency[q], total_duration)
        error_per_quarter = np.append(error_per_quarter, np.sqrt(np.sum(err_up**2))/np.size(err_up))

        #propogating the error
        difference_err = err_up #calc_error_during_subtraction(quarterly_evf_x_energy[q]t, err_up, popt, perr ,total_duration)
    ...
    if(PLOT):
        cbar = plt.colorbar(COLOR, ticks=DAY_RANGE)
        cbar.set_label('$BJD_{TDB}-2454832$',fontsize = 10, rotation=270)
        if(kwargs['save']==True):plt.savefig('log_space_analysis/evf_sub/'+ str(KIC) + '_evf_sub.png')
...
def plot_evf_sub_mean(KIC, time, mean_frequency, mean_frequency_err,  group_size, evf_sub_mean_data,error_per_quarter, target_index,**kwargs):
    ...
    if(PLOT):
        ...
        if(kwargs['save']==True): plt.savefig('log_space_analysis/evf_sub_mean/'+ str(KIC) + '_evf_sub_mean_groupsize_' + str(group_size) + '.png')
...
def main():
    #cycling through three bins: promising, neutral and noise (good, bad, ugly). Code to determine which KIC belong to which bin available on the notebook
        ...
        for line in targets:
            ...
            KIC = line.rstrip('\n')
            print("\nWorking on the energy_vs_frequency analysis for KIC: "+str(KIC)+",   " + str(target_index-1) + "/" +str(target_count)+"\n")
            ...
            quarterly_evf_energy, quarterly_evf_log_frequency, popt, perr, total_duration, time, success  = plot_evf(KIC, files, num_files, showe=SHOWE,errore=ERRORE,save=SAVEPLOT, OK68=OK68_CUTOFF)

            #rarely, plot_evf will throw and exception because curve_fit fails to find a fit, so this is where curve_fit is checked
            if(success):

                #subtracting the fit from each quarter and plotting it
                mean_log_frequency, mean_log_frequency_err, error_per_quarter = plot_evf_sub(KIC, quarterly_evf_energy, quarterly_evf_log_frequency, popt,perr, total_duration,showes=SHOWES,errores=ERRORES, save=SAVEPLOT)
